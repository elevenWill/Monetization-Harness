...
证据足以区分候选方向时，Harness 会直接建议证据支持的方向并说明决定性依据。证据不足时，它不会用个人匹配度或低成本补出一个“最佳方向”，而会给出最小、可执行、能区分主要假设的下一步。只有当你明确要求在已有候选中立即选择时，才会给一个临时的试验顺序，并说明什么结果会改变它；这不是市场排名，也不会在你接受前写成 Workspace Decision。
