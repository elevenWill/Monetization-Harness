...
证据不够排出哪个市场更好时，Harness 仍会给你一个针对当前处境的明确建议，不拿“还不能证明”当挡箭牌。这个内部判断叫 Recommended Bet：它不是市场事实、验证结果或“最佳赛道”，也不会在你确认前被写成 Workspace Decision。它可以综合已有证据、你的能力和限制、可逆性、时间与金钱成本、机会成本和相近案例；用户匹配度可以决定先试什么，但不能冒充市场更好。

用户看到的标题是“我的建议”，默认不超过 800 个中文字，并按四段回答：第一句直接说“如果我是你，我会……”；再说为什么适合你、为什么暂时不选最重要的替代方向，以及做多久、置信度多高、看到什么结果会改主意。理由至少引用你的三条具体情况。信息不足时最多追问一个真正会改变建议的问题；能先给临时建议时就先给。正常回答尽量不用 Stage、FACT、ASSUMPTION、Market Priority 等内部术语，也不会反复堆叠同一种免责声明；需要完整操作细节的执行题不受 800 字限制。
