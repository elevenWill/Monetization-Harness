...
description: Produce auditable external market evidence for monetization decisions by investigating exact and adjacent precedents, failures, user behavior, purchase-trigger events, real deadlines, cost of delay, buyer-originated urgency, competitors, prices, current platform policy, and case transferability. Use before Thinking Skills when a conclusion depends on current market reality; when the user asks whether a direction is viable, proven, permitted, accepted, competitive, worth entering, or likely to be bought now; when a deadline, campaign, launch, filing, rush request, penalty, or delay consequence is part of the opportunity; when they request cases or want to copy one; before substantial spend or long development when research is missing or stale; and for a quick public-market reconnaissance after bootstrapping a concrete monetization project. Reuse fresh scope-matched research instead of searching again.
---
...
- **quick:** For a new public-market project or go/no-go reconnaissance. Target at least one current official/authoritative policy source, two case candidates, one negative signal, one competitor/substitute class, and explicit gaps.
- **standard:** For viability, case requests, experiment preparation, or a material direction. Target at least three independent case candidates, ideally one or two exact cases; at least two negative sources; one current official policy source per target platform; acceptance signals from at least two independent channel types; and real price/transaction signals. These are coverage targets, not a license to fill quotas with weak analogies. Stop conclusion escalation when no exact case is found.
